...
namespace SH_hmjh_RegularEvaluationScoreReport
{
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            cat.Add(new FISCA.Permission.RibbonFeature(Permissions.KeyHmjhRegularEvaluationScoreReport, "定期評量成績單(和美)epost"));
            
            var btn = K12.Presentation.NLDPanels.Student.RibbonBarItems["資料統計"]["報表"]["成績相關報表"]["定期評量成績單(和美)epost"];
            btn.Enable = false;
            K12.Presentation.NLDPanels.Student.SelectedSourceChanged += delegate { btn.Enable = K12.Presentation.NLDPanels.Student.SelectedSource.Count > 0 && Permissions.IsEnableHmjhRegularEvaluationScoreReport; };
                ...
                bkw.DoWork += delegate(object sender, System.ComponentModel.DoWorkEventArgs e)
                {
                    ...
                    try
                    {
                        ...
                        for (int subjectIndex = 1; subjectIndex <= conf.SubjectLimit; subjectIndex++)
                        {
                            ...
                            _dtEpost.Columns.Add("班均標" + subjectIndex);                            
                        }

                        _dtEpost.Columns.Add("加權總分");
                        _dtEpost.Columns.Add("加權總分班排名");
                        _dtEpost.Columns.Add("加權總分班排名母數");
                        _dtEpost.Columns.Add("加權平均");
                        _dtEpost.Columns.Add("類別排名1");
                        _dtEpost.Columns.Add("類別1加權總分排名");
                        _dtEpost.Columns.Add("類別1加權總分排名母數");
                        ...
                        string SchoolAddress = K12.Data.School.Address;
                        string SchoolTel = K12.Data.School.Telephone;
                        ...
                        foreach (DataRow dr in table.Rows)
                        {
                            ...
                            {
                                if(_dtEpost.Columns.Contains(dc.Caption))                                
                                    data[dc.Caption] = dr[dc.Caption];
                            }
